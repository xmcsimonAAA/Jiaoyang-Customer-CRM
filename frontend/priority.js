...
const priorityLabels = {sourceSequence:'序号', customerName:'客户姓名', canonicalName:'标准姓名', twCode:'TW 编号', brokerAccountStatus:'券商开户状态', depositAmount:'入金金额（原表单位）', onSiteOpener:'骄阳现场开户人', insuranceBroker:'保险经纪人', zhongyangWitness:'中阳见证人', customerType:'客户类型', agreementSigned:'原表协议标记', agreementAmountUsd:'协议金额（USD）', jiaoyangOwner:'来源负责人（历史）', notes:'备注说明', assetUsd:'总资产（USD）', quantity:'XMax 持股数', serviceOwner:'客户负责人（全产品共用）',assignmentReason:'分配来源 / 待办',participationIntent:'参与意向',currentOwner:'客户主档负责人', agreementUsd:'协议总额（USD）'};
...
async function renderPriorityBatches(panel) {
  ...
  const update=()=>{
    ...
    panel.querySelector('#p-records').innerHTML=`<p class="p-count">筛选结果 ${rows.length} 条 · 协议金额 ${pnum(rows.reduce((s,r)=>s+Number(r.agreementAmountUsd||0),0))} USD</p><div class="p-table-wrap"><table class="p-table"><thead><tr>${cols.map(c=>`<th>${priorityLabels[c]}</th>`).join('')}<th>来源与操作</th></tr></thead><tbody>${rows.map((r,i)=>`<tr>${cols.map(c=>`<td>${['agreementAmountUsd','depositAmount'].includes(c)?pnum(r[c]):esc(r[c]||'—')}${c==='customerName' && pcanImport()?`<button class="p-link" data-p-edit-details="${i}">修改资料</button>`:''}${c==='customerName' && r.originalCustomerName?`<small>原表姓名：${esc(r.originalCustomerName)}</small>`:''}${c==='customerName' && r.canonicalName && r.canonicalName!==r.customerName?`<small>标准姓名：${esc(r.canonicalName)}</small>`:''}${c==='twCode' && !r.twCode?(pcanImport()?`<button class="p-link p-pending" data-p-identity="${i}">匹配身份</button>`:'<span class="p-pending">待匹配</span>'):''}${c==='assignmentReason' && (!r.serviceOwnerId || r.ownerConflict) && pcanAssign()?`<button class="p-link" data-p-assignment="${i}">立即处理</button>`:''}</td>`).join('')}<td><button class="p-link" data-p-follow="${i}">写跟进</button><button class="p-link" data-p-source="${i}">第 ${r.sourceRow} 行</button>${r.twCode?`<button class="p-link" data-p-customer="${esc(r.twCode)}">客户历史</button>`:''}${pcanImport()?`<button class="p-link" data-p-edit="${i}">${r.twCode?'补充 / 修订':'匹配身份'}</button>`:''}</td></tr>`).join('') || `<tr><td colspan="13">没有符合条件的记录。</td></tr>`}</tbody></table></div>`;
...
function renderPriorityCustomers(panel) {
  ...
  const update=()=>{
    ...
    const cols=['customerName','twCode','brokerAccountStatus','insuranceBroker','jiaoyangOwner','serviceOwner','assignmentReason','assetUsd','quantity','agreementUsd'];
...
async function priorityEdit(row, data) {
  ...
  const editOwner=row.serviceOwnerId?row.serviceOwner:(row.jiaoyangOwner||'');
  ...
  const fields=['customerName','twCode','brokerAccountStatus','depositAmount','onSiteOpener','insuranceBroker','zhongyangWitness','customerType','agreementSigned','agreementAmountUsd','jiaoyangOwner','notes'];
  openModal(`<div class="modal p-modal"><div class="modal-header"><h3>${esc(row.customerName)} · 补充业务资料</h3><button class="close-btn" data-close>×</button></div><form id="p-edit-form"><div class="modal-body"><p>批次 ${esc(priorityState.batch)}。原始文件保留，修订生成新版本。修改负责人会同步更新定增、优先劣后和客户卡片。</p><p>可修正原表姓名。原始 Excel 姓名会保留在来源记录中，修改原因会写入历史。TW 留空可排除错误关联，批次资料及其业务跟进仍保留。后续名单出现新的唯一匹配时会自动补齐。</p>${row.twCode?'<button type="button" class="secondary-btn" id="p-detach">不是同一人，解除当前 TW 关联</button>':''}${!row.twCode && candidates.length?`<div class="notice"><strong>同名候选：点击选择，无需输入编号</strong><div class="p-candidate-list">${candidates.map(c=>`<button type="button" class="secondary-btn" data-identity-code="${esc(c.twCode)}">${esc(c.customerName)} · ${esc(c.twCode)} · ${esc(c.brokerAccountStatus||'状态未提供')}</button>`).join('')}</div></div>`:''}<div class="form-grid">${fields.map(f=> f==='jiaoyangOwner' ? `<div class="field"><label>客户负责人（修改对所有产品生效）</label><select name="jiaoyangOwner" ${state.user.canManageAssignments?'':'disabled'}><option value="">未指派</option>${editOwner && !owners.some(p=>p.name===editOwner)?`<option selected value="${esc(editOwner)}">${esc(editOwner)}（原表姓名，待关联）</option>`:''}${owners.map(p=>`<option value="${esc(p.name)}" ${p.name===editOwner?'selected':''}>${esc(p.name)} · ${esc(p.team)}</option>`).join('')}</select></div>` : `<div class="field"><label>${f==='customerName'?'修正后的客户姓名':priorityLabels[f]}</label><input name="${f}" value="${esc(row[f]??'')}" ${['depositAmount','agreementAmountUsd'].includes(f)?'type="number" min="0" step="any"':''} ${f==='twCode'?'placeholder="从客户名单选择 TW" list="p-tw-options"':''}></div>`).join('')}<div class="field"><label>参与意向</label><select name="participationIntent"><option value="service" ${row.participationIntent!=='interested'?'selected':''}>普通服务 / 尚未明确意向</option><option value="interested" ${row.participationIntent==='interested'?'selected':''}>有意向参与，须领导指派</option></select></div><datalist id="p-tw-options">${priorityState.data.customers.map(c=>`<option value="${esc(c.twCode)}">${esc(c.customerName)}</option>`).join('')}</datalist><div class="field span-2"><label>修订原因</label><input name="reason" required placeholder="例如：核对原始资料后确认姓名，或补充本周负责人"></div></div><p id="p-edit-error" class="p-error" role="alert"></p></div><div class="modal-footer"><button type="button" class="secondary-btn" data-close>取消</button><button class="primary-btn">保存修订</button></div></form></div>`);
  ...
  document.querySelector('#p-edit-form').onsubmit=async e=>{
    ...
    for(const f of fields) if(values[f]!==undefined && values[f]!==String((f==='jiaoyangOwner'?editOwner:row[f])??'')) changes[f]=values[f];
    try {if(!Object.keys(changes).length)throw new Error('还没有修改字段。');await pcall(`batches/${priorityState.batch}/records/${encodeURIComponent(row.recordKey)}`,{expectedRevision:data.revision,expectedCustomerVersion:row.customerVersion,changes,reason:values.reason},'PATCH');closeModal();toast('修订已保存');await priorityRefreshAfterAction();} catch(err){document.querySelector('#p-edit-error').textContent=err.message;setBusyButton(button,false);}
  };
}

function renderPriorityUpload(panel) {
  const remembered=sessionStorage.getItem('priority_import_asof')||pdate();
  const skipBlank=localStorage.getItem('priority_skip_icc_blank_account')!=='false';
  const batchDate=sessionStorage.getItem('priority_import_batch')||'2026-09-09';
  panel.innerHTML=`<div class="p-panel-heading"><div><h4>批量上传本周资料</h4><p>自动识别四种常用表格。先上传 ICC 记录活动；开户后上传客户名单、资产和持仓，系统自动补齐历史活动的 TW 并关联数据。客户名单只读第一个子表，资产只读 ABC。</p></div></div><form id="p-upload-form"><div class="p-upload-controls"><label>资产 / 持仓统计日期<input name="asOf" type="date" value="${esc(remembered)}" required></label><label>本次 ICC 批次日期<input name="batchDate" type="date" value="${esc(batchDate)}"></label></div>${state.view!=='shared-import'?`<div class="p-import-option"><label><input type="checkbox" name="skipIccBlankAccountStatus" ${skipBlank?'checked':''}><strong>跳过未开户客户（ICC 开户状态为空）</strong></label><p>仅导入“是否开券商账户”有内容的行；“已提交”会导入，但不代表已开户或已有 TW。关闭可导入全部。已保存的历史记录不会删除，其他三类表格不受影响。</p></div>`:''}<label class="p-drop" id="p-drop"><strong>把 Excel 拖到这里，或点击选择</strong><span>可一次选择客户名单、资产、XMax 持仓和港安 ICC 文件</span><input id="p-files" type="file" accept=".xlsx" multiple required></label><p id="p-file-names"></p><button class="primary-btn" type="submit">读取并核对变化</button><p id="p-upload-error" class="p-error" role="alert"></p></form><div id="p-preview"></div>`;
  const files=panel.querySelector('#p-files');
  let selected=[],uploadVersion=0;
  const invalidate=()=>{uploadVersion++;priorityState.preview=null;panel.querySelector('#p-preview').innerHTML='';};
  panel.querySelector('#p-upload-form').onchange=()=>{invalidate();const toggle=panel.querySelector('[name=skipIccBlankAccountStatus]');if(toggle)localStorage.setItem('priority_skip_icc_blank_account',String(toggle.checked));};
  ...
  drop.ondrop=e=>{e.preventDefault();drop.classList.remove('dragging');files.files=e.dataTransfer.files;selected=[...files.files];names();invalidate();};
  panel.querySelector('#p-upload-form').onsubmit=async e=>{
    e.preventDefault();invalidate();const requestVersion=uploadVersion;const button=e.submitter;setBusyButton(button,true,'正在读取并核对…');panel.querySelector('#p-upload-error').textContent='';
    try {
      ...
      const result=await pcall('imports/preview',{asOf:values.asOf,batchDates:values.batchDate?[values.batchDate]:[],files:encoded,skipIccBlankAccountStatus:values.skipIccBlankAccountStatus==='on'});
      if(requestVersion!==uploadVersion)throw new Error('导入设置或文件已改变，请重新读取并核对变化。');
      priorityState.preview=result;
...
function priorityAccountFilterSummary(filter) {
  if(!filter)return '';
  return `<div class="p-account-summary"><p><strong>${filter.enabled?'已开启：跳过开户状态为空的行':'已关闭筛选：导入全部 ICC 客户'}</strong> · 原表 ${filter.sourceCount} 人，本次纳入 ${filter.includedCount} 人，跳过 ${filter.skipped.length} 人。</p>${filter.skipped.length?`<details><summary>查看跳过的 ${filter.skipped.length} 人</summary><div class="p-change-list">${filter.skipped.map(r=>`<p>${esc(r.name)} · 原表第 ${r.sourceRow} 行 · 开户状态为空</p>`).join('')}</div><p>需要这些客户时，关闭上方开关后重新读取；也可补充原表开户状态后再次上传。</p></details>`:''}</div>`;
}

function renderPriorityPreview(container) {
  const preview=priorityState.preview;
  container.innerHTML=`<div class="p-preview"><h4>本次上传变化</h4>${priorityIdentitySummary(preview.identityReconciliation)}${preview.sharedCustomers?`<div class="notice"><strong>共用客户池：新增 ${preview.sharedCustomers.createdCount} 人，补关联 ${preview.sharedCustomers.linkedCount} 个 TW。</strong><p>两个业务共用这份名单。新增客户不自动参与产品，负责人在两个产品中共用，已有归属不会被上传覆盖；撤销来源上传不删除共用客户主档。</p>${preview.sharedCustomers.conflicts.map(c=>`<p>${esc(c.twCode)}：${esc(c.reason)}</p>`).join('')}</div>`:''}${preview.datasets.map(d=>`<article><b>${esc(d.label)} · ${d.date}</b>${priorityAccountFilterSummary(d.iccAccountFilter)}<span>${d.records} 条记录${d.kind==='icc'?` · ${d.participants} 条参与 · ${pnum(d.agreementUsd)} USD · ${d.pending} 条身份待匹配`:''}${d.kind==='assets'?` · ${pnum(d.assetUsd)} USD`:''}${d.kind==='secondary'?` · ${pnum(d.quantity)} 股`:''}</span><p>${d.duplicate?'与当前资料相同，将自动跳过':`${d.changes.length} 条新增或变化`}${d.missingCount?`；${d.missingCount} 条未纳入本次导入，${d.kind==='icc'?'保留原批次记录':'当期快照按新文件列示，旧版可追溯'}`:''}</p>${!d.duplicate&&d.changes.length?`<details><summary>查看变化明细</summary><div class="p-change-list">${d.changes.map(c=>`<p><b>${esc(c.name)} ${esc(c.twCode||'')}</b>${d.kind==='icc'?`<small>${c.twCode?'关联已有 TW 客户（不是新增客户）':'身份待确认，未关联已有客户'}</small>`:''} ${c.fields.map(f=>`${esc(priorityLabels[f]||f)}${c.before?`：${esc(c.before[f]??'空白')} → ${esc(c.after[f]??'空白')}`:''}`).join('；')}</p>`).join('')}</div></details>`:''}</article>`).join('')}${preview.issues.length && !preview.identityReconciliation?`<details open><summary>${preview.issues.length} 条身份待确认</summary><p>可以先保存活动。以后上传更新的客户名单，系统会自动重试关联；仅同名冲突需到“匹配身份”选择客户，无需逐条输入编号。</p>${preview.issues.map(i=>`<p>${esc(i.name)} · 原表第 ${i.sourceRow} 行 · ${i.candidates.length?'同名候选，尚未关联（需核对 TW）':'身份待确认，尚未关联'}${i.candidates.length?`（${i.candidates.map(esc).join('、')}）`:''}</p>`).join('')}</details>`:''}<button id="p-commit" class="primary-btn">确认保存本次资料</button><p id="p-commit-error" class="p-error" role="alert"></p></div>`;
...
async function renderPriorityHistory(panel) {
  ...
  panel.innerHTML=`<div class="p-panel-heading"><div><h4>上传与修订历史</h4><p>保留来源和历史版本。已有后续修订的资料，需要先撤销后续修订。</p></div></div><div class="p-history">${jobs.map(j=>`<article><div><b>${esc(j.createdAt)}</b><p>${j.datasets.map(d=>`${esc(d.label)} ${d.date}（${d.records} 条${d.iccAccountFilter?.enabled?`，跳过开户状态空白 ${d.iccAccountFilter.skipped.length} 条`:''}）`).join('；')}</p><span>${j.status==='rolled_back'?'已撤销':'已保存'}</span></div>${j.status==='committed'?`<button class="secondary-btn" data-p-rollback="${j.id}">撤销本次</button>`:''}</article>`).join('')||'<p>尚无上传记录。</p>'}</div><p id="p-history-error" class="p-error" role="alert"></p>`;
