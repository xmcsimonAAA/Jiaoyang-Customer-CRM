...
async function renderPriorityBatches(panel) {
  ...
  const update=()=>{
    ...
    panel.querySelector('#p-records').innerHTML=`<p class="p-count">筛选结果 ${rows.length} 条 · 协议金额 ${pnum(rows.reduce((s,r)=>s+Number(r.agreementAmountUsd||0),0))} USD</p><div class="p-table-wrap"><table class="p-table"><thead><tr>${cols.map(c=>`<th>${priorityLabels[c]}</th>`).join('')}<th>来源与操作</th></tr></thead><tbody>${rows.map((r,i)=>`<tr>${cols.map(c=>`<td>${['agreementAmountUsd','depositAmount'].includes(c)?pnum(r[c]):esc(r[c]||'—')}${c==='customerName' && r.canonicalName && r.canonicalName!==r.customerName?`<small>标准姓名：${esc(r.canonicalName)}</small>`:''}${c==='twCode' && !r.twCode?'<span class="p-pending">待匹配</span>':''}</td>`).join('')}<td><button class="p-link" data-p-follow="${i}">写跟进</button><button class="p-link" data-p-source="${i}">第 ${r.sourceRow} 行</button>${r.twCode?`<button class="p-link" data-p-customer="${esc(r.twCode)}">客户历史</button>`:''}${pcanImport()?`<button class="p-link" data-p-edit="${i}">${r.twCode?'补充 / 修订':'匹配身份'}</button>`:''}</td></tr>`).join('') || `<tr><td colspan="13">没有符合条件的记录。</td></tr>`}</tbody></table></div>`;
