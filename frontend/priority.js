...
function priorityIdentitySummary(result) {
  ...
  const rows=items=>items.map(i=>`<li><strong>${esc(i.name)}</strong><span>活动批次 ${esc((i.key || '').replace('icc:',''))} · 原表第 ${esc(i.sourceRow)} 行</span><p>${esc(i.reason)}${i.candidates?.length?`（${i.candidates.map(esc).join('、')}）`:''}</p></li>`).join('');
  return `<section class="p-identity-summary" aria-label="历史活动身份关联结果">
    <div class="p-identity-cards">
      ...
      <div class="p-identity-card ${conflicts.length?'p-identity-conflict':''}"><span>需要处理的身份冲突</span><strong>${conflicts.length}<small> 条</small></strong><p>${conflicts.length?'保存后到对应批次的“匹配身份”选择客户。':'目前没有需要处理的身份冲突。'}</p></div>
    </div>
    <p class="p-identity-context">以下是本次核对涉及的历史活动记录，可能跨多个批次。等待和冲突记录会保留，不影响其他资料保存。</p>
    ${conflicts.length?`<details class="p-identity-details p-identity-conflict" open><summary>需要处理：${conflicts.length} 条身份冲突</summary><ul>${rows(conflicts)}</ul></details>`:''}
