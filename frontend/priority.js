...
async function renderPriority(content) {
  ...
  content.innerHTML = `<div class="priority-page"><div class="priority-heading"><div><span class="eyebrow">优先劣后 · 每周数据</span><h3>本周资料，一处查清</h3><p>按批次查协议与负责人，按客户查资产和 XMax 持仓。</p></div><label>查看截至日期<input id="p-asof" type="date" value="${esc(priorityState.asOf)}" aria-label="查看截至日期"></label></div>
    <div class="p-metrics"><article><span>参与客户（去重）</span><strong>${s.participants}<small> 人</small></strong><p>${s.participationRecords} 条参与记录${s.unmatchedParticipations ? ` · ${s.unmatchedParticipations} 条身份待确认` : ''}</p></article><article><span>协议总额 · USD</span><strong>${pnum(s.agreementUsd)}</strong><p>按已填协议金额统计</p></article><article><span>XMax 当前持仓客户</span><strong>${s.secondaryParticipants}<small> 人</small></strong><p>${data.dates.secondary || '尚未上传'} · ${pnum(s.quantity)} 股</p></article><article><span>客户总资产 · USD</span><strong>${pnum(s.assetsUsd)}</strong><p>${data.dates.assets || '尚未上传'} · 同一 TW 账户相加</p></article></div>
    <div class="p-tabs" role="tablist">${[['batches','批次记录'],['customers','客户综合查询'],['upload','上传与核对'],['history','上传历史'],['bindings','服务绑定']].filter(([key]) => key==='bindings' ? state.user.customerScope==='all' && state.user.canManageAssignments && state.user.canManageAdvisorBindings : !['upload','history'].includes(key) || pcanImport()).map(([key,label])=>`<button role="tab" aria-selected="${priorityState.tab === key}" data-p-tab="${key}">${label}${key==='batches' && s.pending ? ` <span>${s.pending} 待匹配</span>`:''}</button>`).join('')}</div>
  ...
  content.querySelectorAll('[data-p-tab]').forEach(b=>b.onclick=async()=>{priorityState.tab=b.dataset.pTab; await renderPriority(content);});
  ...
  if (priorityState.tab==='bindings') await renderPriorityBindings(panel);
...
async function renderPriorityBatches(panel) {
  ...
  const update=()=>{
    ...
    panel.querySelector('#p-records').innerHTML=`<p class="p-count">筛选结果 ${rows.length} 条 · 协议金额 ${pnum(rows.reduce((s,r)=>s+Number(r.agreementAmountUsd||0),0))} USD</p><div class="p-table-wrap"><table class="p-table"><thead><tr>${cols.map(c=>`<th>${priorityLabels[c]}</th>`).join('')}<th>来源与操作</th></tr></thead><tbody>${rows.map((r,i)=>`<tr>${cols.map(c=>`<td>${['agreementAmountUsd','depositAmount'].includes(c)?pnum(r[c]):esc(r[c]||'—')}${c==='customerName' && r.canonicalName && r.canonicalName!==r.customerName?`<small>标准姓名：${esc(r.canonicalName)}</small>`:''}${c==='twCode' && !r.twCode?'<span class="p-pending">待匹配</span>':''}</td>`).join('')}<td><button class="p-link" data-p-source="${i}">第 ${r.sourceRow} 行</button>${r.twCode?`<button class="p-link" data-p-customer="${esc(r.twCode)}">客户历史</button>`:''}${pcanImport()?`<button class="p-link" data-p-edit="${i}">${r.twCode?'补充 / 修订':'匹配身份'}</button>`:''}</td></tr>`).join('') || `<tr><td colspan="13">没有符合条件的记录。</td></tr>`}</tbody></table></div>`;
    ...
    panel.querySelectorAll('[data-p-edit]').forEach(b=>b.onclick=()=>priorityEdit(rows[Number(b.dataset.pEdit)],data));
  };
...
async function priorityCustomerDetail(code) {
  try {
...
function renderPriorityPreview(container) {
  ...
  container.innerHTML=`<div class="p-preview"><h4>本次上传变化</h4>${preview.datasets.map(d=>`<article><b>${esc(d.label)} · ${d.date}</b><span>${d.records} 条记录${d.kind==='icc'?` · ${d.participants} 条参与 · ${pnum(d.agreementUsd)} USD · ${d.pending} 条身份待匹配`:''}${d.kind==='assets'?` · ${pnum(d.assetUsd)} USD`:''}${d.kind==='secondary'?` · ${pnum(d.quantity)} 股`:''}</span><p>${d.duplicate?'与当前资料相同，将自动跳过':`${d.changes.length} 条新增或变化`}${d.missingCount?`；${d.missingCount} 条未出现在新文件中，${d.kind==='icc'?'保留原批次记录':'当期快照按新文件列示，旧版可追溯'}`:''}</p>${!d.duplicate&&d.changes.length?`<details><summary>查看变化明细</summary><div class="p-change-list">${d.changes.map(c=>`<p><b>${esc(c.name)} ${esc(c.twCode||'')}</b> ${c.fields.map(f=>`${esc(priorityLabels[f]||f)}${c.before?`：${esc(c.before[f]??'空白')} → ${esc(c.after[f]??'空白')}`:''}`).join('；')}</p>`).join('')}</div></details>`:''}</article>`).join('')}${preview.issues.length?`<details open><summary>${preview.issues.length} 条身份待确认</summary><p>先保存完整活动资料，随后在“批次记录 → 身份待匹配”中补充 TW。统计会标明尚未匹配的参与记录。</p>${preview.issues.map(i=>`<p>${esc(i.name)} · 原表第 ${i.sourceRow} 行 · ${i.status==='ambiguous'?'存在同名客户':'未找到同名 TW'}${i.candidates.length?`（${i.candidates.map(esc).join('、')}）`:''}</p>`).join('')}</details>`:''}<button id="p-commit" class="primary-btn">确认保存本次资料</button><p id="p-commit-error" class="p-error" role="alert"></p></div>`;
  container.querySelector('#p-commit').onclick=async e=>{setBusyButton(e.target,true,'正在保存…');try{await pcall(`imports/${preview.id}/commit`,{});priorityState.preview=null;priorityState.tab='batches';priorityState.asOf='';await renderPriority(document.querySelector('#content'));toast('本次资料已保存');}catch(err){container.querySelector('#p-commit-error').textContent=err.message;setBusyButton(e.target,false);}};
}
