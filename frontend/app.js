...
function renderHonganActivityPreview(workspace, preview) {
  ...
  const sample = (items, label) => items?.length ? `<details class="hongan-import-list"><summary>${label}（${items.length}${items.length >= 200 ? "+" : ""}）</summary><div>${items.slice(0, 8).map((item) => `<span>${esc(item.name)}${item.targetAdvisor ? ` · ${esc(item.targetAdvisor)}` : item.advisors?.length ? ` · ${esc(item.advisors.join("、"))}` : ""}${item.sourceAdvisors?.length ? ` · 原表骄阳：${esc(item.sourceAdvisors.join("、"))}` : ""}</span>`).join("")}</div></details>` : "";
  workspace.innerHTML = `<div class="result-box hongan-import-banner"><strong>已识别港安活动分表</strong><div class="hint">${esc(preview.sheetName || "")}${preview.sheetStats?.length ? ` · ${preview.sheetStats.map((item) => `${esc(item.name)}（${item.rows}）`).join("、")}` : ""}</div><div class="hint import-normalization-note">${esc(preview.textNormalization || "繁体中文已统一转换为简体中文")}</div></div><section class="hongan-import-summary"><div class="hongan-import-stats">${stat("活动记录", activity.totalRows)}${stat("唯一客户", activity.uniqueNames)}${stat("可自动补全", counts.autoFill, "positive")}${stat("已有一致顾问", counts.unchanged)}${stat("需人工复核", (counts.conflicts || 0) + (counts.ambiguous || 0), "warning")}${stat("系统未找到", counts.unmatched, "muted")}</div>${sample(activity.conflicts, "顾问冲突")}${sample(activity.ambiguous, "同名多条记录")}${sample(activity.unmatched, "未匹配客户")}</section><div class="toolbar import-actions"><label class="import-consent hongan-import-consent"><input type="checkbox" id="confirm-hongan-activity"><span>我确认只补全唯一匹配的港安顾问，并按原表骄阳顾问分配仍处于待分配的客户；不覆盖已有负责人、不新建客户。</span></label><button class="primary-btn" id="commit-import" ${preview.truncated || !state.user.canManageAdvisorBindings ? "disabled" : ""}>确认补全并分配</button></div><div class="table-wrap import-sample"><table><thead><tr>${preview.headers.map((header) => `<th>${esc(header)}</th>`).join("")}</tr></thead><tbody>${preview.rows.slice(0, 5).map((row) => `<tr>${preview.headers.map((header) => `<td>${esc(row[header])}</td>`).join("")}</tr>`).join("")}</tbody></table></div>`;
