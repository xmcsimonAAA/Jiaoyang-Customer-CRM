const state = {
  ...
  customers: [], customerPage: 1, search: "", workflow: "all", metric: "", stage: "", ownerId: "", accountStatus: "", intentStatus: "", placementStatus: "", source: "", honganAdvisor: "", contactState: "", detail: null, importPreview: null, dashboardAssistantFocus: false,
};
const app = document.querySelector("#app");
const VIEWS = new Set(["dashboard", "customers", "placement", "followups", "imports", "fields", "bindings", "permissions", "audit"]);
const WORKSPACE_STATE_KEY = "jy_customer_workspace_state";
const WORKSPACE_STATE_FIELDS = ["view", "customerPage", "search", "workflow", "metric", "stage", "ownerId", "accountStatus", "intentStatus", "placementStatus", "source", "honganAdvisor", "contactState"];
...
async function api(path, options = {}) {
  ...
  if (!response.ok) { const fallback = response.status >= 500 ? "服务器未能解析该文件，请确认文件未损坏，或另存为新的 .xlsx 后重试。" : "操作失败，请稍后重试。"; const message = typeof data.detail === "string" ? data.detail : data.detail?.message || fallback; throw Object.assign(new Error(message), {status: response.status, data}); }
  return data;
}

function renderLogin(error = "", authConfig = {}) {
  if (authConfig.passwordLoginEnabled === false) {
    const entryUrl = authConfig.muskzoomEntryUrl || "https://muskzoom.com";
...
function renderShell() {
  ...
  document.querySelector("#logout").addEventListener("click", async () => { await api("/api/session", {method:"DELETE"}).catch(() => {}); localStorage.removeItem("jy_customer_token"); state.token = null; state.user = null; await showLogin(); });
}

async function navigate(view, options = {}) {
  if (!VIEWS.has(view)) view = "dashboard";
  if (view === "bindings" && !["supervisor", "admin", "developer"].includes(state.user.rolePermission)) view = "dashboard";
  saveWorkspaceState();
  const previousView = state.view; state.view = view; if (view === "customers" && previousView !== "customers" && !options.preserveWorkflow) { state.workflow = "all"; state.metric = ""; }
...
async function renderDashboardLegacy(content) {
  ...
  content.querySelectorAll("[data-workflow]").forEach((button) => button.addEventListener("click", () => { state.metric = button.dataset.workflow; state.workflow = "all"; state.search = ""; state.stage = ""; state.ownerId = ""; state.accountStatus = ""; state.intentStatus = ""; state.placementStatus = ""; state.source = ""; state.honganAdvisor = ""; state.contactState = ""; state.customerPage = 1; navigate("customers", {preserveWorkflow:true}); }));
}
...
function dashboardResetCustomerFilters() {
  ...
  state.intentStatus = ""; state.placementStatus = ""; state.source = ""; state.honganAdvisor = ""; state.contactState = ""; state.customerPage = 1;
}
...
async function renderCustomers(content) {
  const query = new URLSearchParams({
    ...
    contactState: state.contactState, page: state.customerPage, pageSize: 100,
  });
  ...
  const filterCount = [state.search, state.stage, state.ownerId, state.accountStatus, state.intentStatus, state.placementStatus, state.source, state.honganAdvisor, state.contactState, state.metric].filter(Boolean).length;
  ...
  document.querySelector("#clear-customer-filters").addEventListener("click", () => { state.search = ""; state.stage = ""; state.ownerId = ""; state.accountStatus = ""; state.intentStatus = ""; state.placementStatus = ""; state.source = ""; state.honganAdvisor = ""; state.contactState = ""; state.metric = ""; state.workflow = "all"; state.customerPage = 1; renderCustomers(content); });
...
async function renderBatches(content) {
  const data = await api("/api/batches"); const canManage = ["supervisor", "admin", "developer"].includes(state.user.rolePermission);
...
function customerTable(rows, editable = false) {
  ...
  const canManageHonganAdvisor = editable && ["supervisor", "admin", "developer"].includes(state.user.rolePermission);
  return rows.length ? `<table class="customer-grid"><thead><tr><th class="sticky-col">客户</th><th>微信昵称</th><th>联系方式</th><th>港安顾问<br><small>外部引荐</small></th><th>开户状态</th><th>定增推进</th><th>当前骄阳负责人</th>${fields.map((field) => `<th>${esc(field.label)}</th>`).join("")}<th>最后更新</th></tr></thead><tbody>${rows.map((row) => {
    ...
    return `<tr><td class="sticky-col">${displayName ? `<button class="link-btn" data-open-customer="${esc(row.id)}">${esc(displayName)}</button>` : ""}</td><td class="customer-nickname">${nickname ? `<button class="link-btn" data-open-customer="${esc(row.id)}">${esc(nickname)}</button>` : "-"}</td><td>${esc(row.phone || "-")}${row.email ? `<div class="hint">${esc(row.email)}</div>` : ""}</td><td class="hongan-advisor-cell">${canManageHonganAdvisor ? coreGridText(row,"hkAdvisor",honganAdvisor,"未填写") : esc(honganAdvisor || "-")}</td><td>${editable ? coreGridSelect(row,"accountStatus",state.meta.accountStatuses,row.account_status) : tag(row.account_status,toneFor(row.account_status))}</td><td>${editable ? coreGridSelect(row,"placementStatus",state.meta.placementStatuses,row.placement_status) : tag(row.placement_status,toneFor(row.placement_status))}</td><td>${esc(row.owner_name)}<div class="hint">${esc(row.owner_team)}${row.source_advisor_label ? `<br>原表骄阳顾问：${esc(row.source_advisor_label)}` : ""}</div></td>${fields.map((field) => `<td>${editable ? gridFieldControl(row, field) : esc(row.custom_values?.[field.id] || "-")}</td>`).join("")}<td class="hint">${fmt(row.updated_at)}</td></tr>`;
...
function renderImports(content) { const admin = ["admin", "developer"].includes(state.user.rolePermission); content.innerHTML = `<div class="section-heading entry-heading"><div><div class="eyebrow">DAILY CUSTOMER INTAKE</div><h3>录入中心</h3><p>日常新增直接进入统一客户池；历史数据迁移使用批量导入。</p></div><span class="entry-status"><i></i>数据实时同步</span></div><div class="entry-workspace"><section class="entry-primary"><div class="entry-primary-copy"><span class="entry-overline">日常工作</span><h4>先记住客户，再逐步完善进度</h4><p>姓名、微信昵称、手机号和当前情况即可新建。开户、定增和自定义字段可在后续跟进中继续补充。</p></div><button class="entry-cta" id="single-entry"><span>＋</span>新增一位客户</button></section><div class="entry-cards"><button class="entry-card" id="bulk-entry" ${state.user.canImportCustomers ? "" : "disabled"}><span>⇧</span><strong>批量导入历史数据</strong><small>${state.user.canImportCustomers ? "Excel / CSV 先预览列映射，再确认并入客户表。" : "此账号尚未获得批量导入权限，请联系管理员开通。"}</small><b>${state.user.canImportCustomers ? "打开导入工具 →" : "暂未开通"}</b></button><article class="entry-card entry-note"><span>◎</span><strong>录入后的处理方式</strong><small>重复客户不会自动合并；系统会提示冲突，由小组长及以上人员确认处理。</small></article></div></div><section class="section import-workspace" id="import-workspace"><div class="section-header"><div><h3>历史数据迁移</h3><span class="hint">仅在需要导入 Excel 或 CSV 时展开。</span></div>${state.user.canImportCustomers ? `<div class="heading-actions"><button class="secondary-btn" id="download-import-template">下载导入模板</button><button class="secondary-btn" id="open-import">选择表格</button></div>` : ""}</div><div class="empty">日常客户请使用上方的“新增一位客户”。</div></section>${admin ? `<section class="section import-history" id="import-history"><div class="empty">正在读取导入记录...</div></section>` : ""}`; document.querySelector("#single-entry").addEventListener("click", () => openQuickCustomerForm()); document.querySelector("#bulk-entry")?.addEventListener("click", () => renderBulkImport()); document.querySelector("#open-import")?.addEventListener("click", () => renderBulkImport()); document.querySelector("#download-import-template")?.addEventListener("click", downloadImportTemplate); if (admin) renderImportHistory(); }
...
async function renderImportHistory() { const workspace = document.querySelector("#import-history"); if (!workspace) return; try { const data = await api("/api/imports"); workspace.innerHTML = `<div class="section-header"><div><h3>导入记录</h3><span class="hint">仅管理员可撤回尚未发生后续业务动作的导入记录。</span></div><span class="hint">最近 ${data.items.length} 次</span></div><div class="table-wrap"><table><thead><tr><th>时间</th><th>文件</th><th>导入人</th><th>结果</th><th>状态</th><th></th></tr></thead><tbody>${data.items.length ? data.items.map((item) => { const status = item.rolled_back_at ? "已撤回" : item.created_count ? "可检查" : "无新增"; return `<tr><td class="hint">${fmt(item.created_at)}</td><td>${esc(item.filename)}</td><td>${esc(item.imported_by_name)}</td><td>新增 ${item.created_count} · 冲突 ${item.conflict_count} · 错误 ${item.error_count}</td><td>${tag(status, item.rolled_back_at ? "gray" : item.created_count ? "amber" : "gray")}</td><td>${!item.rolled_back_at && item.created_count ? `<button class="secondary-btn" data-rollback-import="${esc(item.id)}">撤回</button>` : ""}</td></tr>`; }).join("") : `<tr><td colspan="6"><div class="empty">暂无批量导入记录</div></td></tr>`}</tbody></table></div>`; workspace.querySelectorAll("[data-rollback-import]").forEach((button) => button.addEventListener("click", () => rollbackImport(button.dataset.rollbackImport))); } catch (err) { workspace.innerHTML = `<div class="empty">${esc(err.message)}</div>`; } }
async function rollbackImport(jobId) { if (!window.confirm("仅撤回导入后尚未编辑、跟进或录入持仓的客户。确定继续吗？")) return; try { const data = await api(`/api/imports/${encodeURIComponent(jobId)}/rollback`, {method:"POST"}); const message = data.protected.length ? `已撤回 ${data.archived.length} 条，保护 ${data.protected.length} 条` : `已撤回 ${data.archived.length} 条`; toast(message); renderImportHistory(); } catch (err) { toast(err.message); } }
...
function renderImportDiagnostics(workspace, preview) {
  ...
  const profile = preview.profile === "hongan_master" ? `<span class="import-profile">港安客户总表识别模式</span>` : "";
  const ownerChoices = state.meta.ownerChoices || state.meta.owners || [];
  const ownerSelection = state.user.rolePermission === "manager" ? "" : `<div class="import-owner-select"><label>未匹配顾问的处理方式</label><select id="import-default-owner">${ownerChoices.map((owner) => `<option value="${esc(owner.id)}" ${owner.id === "unassigned" ? "selected" : ""}>${esc(owner.name)}${owner.team ? ` · ${esc(owner.team)}` : ""}</option>`).join("")}</select><small>原表顾问会单独保留；未匹配到系统账号时，建议先进入待分配池。</small></div>`;
  ...
  const aliasMapping = state.user.canManageCustomerFields && preview.unresolvedAdvisorAliases?.length ? `<details class="mapping-details advisor-alias-mapping" open><summary>历史顾问账号映射</summary><div class="hint">映射会保存，下次导入遇到同一历史姓名时自动复用。</div><div class="advisor-alias-list">${preview.unresolvedAdvisorAliases.map((alias) => `<label><span>${esc(alias)}</span><select data-advisor-alias="${esc(alias)}"><option value="">暂不映射</option>${aliasOptions}</select></label>`).join("")}</div></details>` : "";
  const quality = preview.dataQuality || {};
  const qualitySummary = `<div class="import-quality"><span>可识别客户 ${Math.max(0, Number(preview.totalRows || 0) - Number(quality.missingDisplayNameRows || 0))} 条</span><span>仅有微信昵称 ${Number(quality.nicknameFallbackRows || 0)} 条</span><span>待补联系方式 ${Number(quality.unidentifiedRows || 0)} 条</span></div>`;
  const unidentifiedConsent = quality.unidentifiedRows ? `<label class="import-consent"><input type="checkbox" id="allow-unidentified-rows"><span>我确认这批历史记录可能没有手机号或邮箱，允许导入并在系统中继续补充。</span></label>` : "";
  const diagnostics = `<section class="import-diagnostics">${profile}${qualitySummary}${ownerSelection}${warningRows ? `<ul>${warningRows}</ul>` : `<p class="hint">未发现需要人工处理的占位值或异常列。</p>`}${unidentifiedConsent}${preview.holdingSnapshots?.length ? `<p class="hint">将自动写入 ${preview.holdingSnapshots.map((item) => esc(item.snapshotDate)).join("、")} 的持仓快照。</p>` : ""}${aliasMapping}${mappingSection}</section>`;
...
async function commitImport() {
  ...
  const rows = preview.rows.map((raw) => {
  ...
  const ownerId = state.user.rolePermission === "manager" ? state.user.id : document.querySelector("#import-default-owner")?.value;
  const allowUnidentifiedRows = Boolean(document.querySelector("#allow-unidentified-rows")?.checked);
  try { const data = await api("/api/imports/commit", {method:"POST", body: JSON.stringify({filename:file.name, ownerId, advisorAliasMappings, allowUnidentifiedRows, rows})}); const potentialCount = data.created.reduce((total, item) => total + (item.potentialDuplicates?.length || 0), 0); document.querySelector("#bulk-preview").insertAdjacentHTML("afterbegin", `<div class="result-box"><strong>导入完成</strong><div class="hint">新增 ${data.created.length} 条 · 冲突 ${data.conflicts.length} 条 · 错误 ${data.errors.length} 条${data.dataQuality?.unidentifiedRowsImported ? ` · 无联系方式 ${data.dataQuality.unidentifiedRowsImported} 条` : ""}${potentialCount ? ` · 发现 ${potentialCount} 条可能重名记录，请人工确认` : ""}</div></div>`); toast("数据已并入客户数据表"); if (["admin", "developer"].includes(state.user.rolePermission)) renderImportHistory(); } catch (err) { toast(err.message); }
}
...
async function renderPermissions(content) { const data = await api("/api/admin/users"); content.innerHTML = `<div class="notice">权限来自 MuskZoom 角色，下面的开关只控制客户模块能力，可逐人单独开通。</div><section class="section"><div class="section-header"><h3>客户模块权限</h3><span class="hint">${data.items.length} 个账号</span></div><div class="table-wrap"><table class="permission-table"><thead><tr><th>成员</th><th>角色 / 团队</th><th>批量导入</th><th>管理表头</th><th>完整导出</th></tr></thead><tbody>${data.items.map((item) => `<tr><td><strong>${esc(item.name)}</strong><div class="hint">${esc(item.username)}</div></td><td>${tag(item.roleLabel, roleTone(item.role))}<div class="hint">${esc(item.team)}</div></td><td><label class="switch"><input type="checkbox" data-permission="import" data-user="${esc(item.id)}" ${item.canImportCustomers ? "checked" : ""}>允许</label></td><td><label class="switch"><input type="checkbox" data-permission="fields" data-user="${esc(item.id)}" ${item.canManageCustomerFields ? "checked" : ""}>允许</label></td><td><label class="switch"><input type="checkbox" data-permission="export" data-user="${esc(item.id)}" ${item.canExportAll ? "checked" : ""}>允许</label></td></tr>`).join("")}</tbody></table></div></section>`; content.querySelectorAll("[data-permission]").forEach((input) => input.addEventListener("change", () => savePermission(input, data.items.find((item) => item.id === input.dataset.user)))); }
async function savePermission(input, item) { const importInput = document.querySelector(`[data-permission=import][data-user="${CSS.escape(item.id)}"]`); const fieldsInput = document.querySelector(`[data-permission=fields][data-user="${CSS.escape(item.id)}"]`); const exportInput = document.querySelector(`[data-permission=export][data-user="${CSS.escape(item.id)}"]`); try { await api(`/api/admin/users/${encodeURIComponent(item.id)}/permissions`, {method:"PATCH", body: JSON.stringify({canImportCustomers: importInput.checked, canManageCustomerFields:fieldsInput.checked, canExportAll: exportInput.checked})}); toast("权限已更新"); } catch (err) { input.checked = !input.checked; toast(err.message); } }
...
function openQuickCustomerForm() {
  ...
  const ownerChoices = state.meta.ownerChoices || state.meta.owners || [];
  const ownerField = state.user.rolePermission === "manager" ? "" : `<div class="field"><label>当前骄阳负责人 *</label><select name="ownerId" required>${ownerChoices.map((owner) => `<option value="${esc(owner.id)}" ${owner.id === "unassigned" ? "selected" : ""}>${esc(owner.name)}${owner.team ? ` · ${esc(owner.team)}` : ""}</option>`).join("")}</select></div>`;
  const canManageHonganAdvisor = ["supervisor", "admin", "developer"].includes(state.user.rolePermission);
...
function renderDrawer() {
  ...
  const canManage = ["supervisor", "admin", "developer"].includes(state.user.rolePermission);
...
function openAssignFormNew(item) {
  const owners = state.meta.owners.filter((owner) => owner.id !== item.owner_id);
