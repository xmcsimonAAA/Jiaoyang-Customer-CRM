const state = {
  ...
  customers: [], customerPage: 1, search: "", workflow: "all", metric: "", stage: "", ownerId: "", accountStatus: "", intentStatus: "", placementStatus: "", source: "", honganAdvisor: "", contactState: "", importJobId: "", importJobMode: "all", detail: null, importPreview: null, dashboardAssistantFocus: false,
...
function renderImportDiagnostics(workspace, preview) {
  ...
  const profile = preview.profile === "hongan_master" ? `<span class="import-profile">港安客户总表识别模式</span>` : preview.importProfile === "asset" ? `<span class="import-profile">券商资产更新模式</span>` : preview.importProfile === "holding" ? `<span class="import-profile">持仓快照更新模式</span>` : "";
  const ownerChoices = state.meta.ownerChoices || state.meta.owners || [];
...
async function commitImport() {
  ...
  try {
    const data = await api("/api/imports/commit", {method:"POST", body: JSON.stringify({filename:file.name, ownerId, mode: importProfile === "standard" && preview.dataQuality?.hasTwSnapshot ? "snapshot" : "append", importProfile, advisorAliasMappings, allowUnidentifiedRows, rows})});
