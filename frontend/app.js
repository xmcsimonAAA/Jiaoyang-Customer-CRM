...
async function renderImportHistory() { const workspace = document.querySelector("#import-history"); if (!workspace) return; try { const data = await api("/api/imports"); workspace.innerHTML = `<div class="section-header"><div><h3>导入记录</h3><span class="hint">仅管理员可撤回尚未发生后续业务动作的新增客户；快照更新会保留审计记录。</span></div><span class="hint">最近 ${data.items.length} 次</span></div><div class="table-wrap"><table><thead><tr><th>时间</th><th>文件</th><th>导入人</th><th>结果</th><th>状态</th><th></th></tr></thead><tbody>${data.items.length ? data.items.map((item) => { const changed = Number(item.created_count || 0) + Number(item.updated_count || 0); const status = item.rolled_back_at ? "已撤回" : changed ? "可检查" : "无变化"; return `<tr><td class="hint">${fmt(item.created_at)}</td><td>${esc(item.filename)}</td><td>${esc(item.imported_by_name)}</td><td>新增 ${item.created_count} · 更新 ${item.updated_count || 0} · 冲突 ${item.conflict_count} · 错误 ${item.error_count}</td><td>${tag(status, item.rolled_back_at ? "gray" : changed ? "amber" : "gray")}</td><td>${!item.rolled_back_at && item.created_count ? `<button class="secondary-btn" data-rollback-import="${esc(item.id)}">撤回</button>` : ""}</td></tr>`; }).join("") : `<tr><td colspan="6"><div class="empty">暂无批量导入记录</div></td></tr>`}</tbody></table></div>`; workspace.querySelectorAll("[data-rollback-import]").forEach((button) => button.addEventListener("click", () => rollbackImport(button.dataset.rollbackImport))); } catch (err) { workspace.innerHTML = `<div class="empty">${esc(err.message)}</div>`; } }
...
function renderImportDiagnostics(workspace, preview) {
  ...
  const snapshotMode = preview.dataQuality?.hasTwSnapshot ? `<p class="hint import-snapshot-note">已识别 TW 编号：确认后按券商全量快照合并，已有客户只更新券商状态，新 TW 才新增。</p>` : "";
