...
function openImportReviewResolver(item) {
  ...
  const rawDetails = [item.sourceSheet ? `来源分表：${item.sourceSheet}` : "", item.sourceRow ? `来源行：第 ${item.sourceRow} 行` : "", item.rows ? `同名记录数：${item.rows}` : "", item.quantity != null ? `持仓数量：${item.quantity}` : "", item.detail?.message || (typeof item.detail === "string" ? item.detail : "")].filter(Boolean).join(" · ");
