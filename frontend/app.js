...
async function renderImportReviews(content) {
  try {
    const data = await api("/api/import-reviews");
    ...
    content.innerHTML = `<div class="section-heading"><div><div class="eyebrow">IMPORT QUALITY CONTROL</div><h3>导入复核</h3><p>把不同批次中需要人工判断的记录集中到这里，已成功写入的客户不会重复显示。</p></div><span class="entry-status ${data.pendingCount ? "is-warning" : ""}"><i></i>${data.pendingCount ? `待处理 ${data.pendingCount} 条` : "暂无待处理"}</span></div><section class="review-summary"><div><b>${data.pendingCount || 0}</b><span>待处理</span></div><div><b>${counts.ambiguous}</b><span>同名待确认</span></div><div><b>${counts.unmatched}</b><span>未匹配</span></div><div><b>${counts.conflict + counts.error}</b><span>冲突 / 错误</span></div></section>${items.length ? `<section class="section review-list"><div class="section-header"><div><h3>待复核记录</h3><span class="hint">按导入时间倒序；处理后记录会从默认列表中移除。</span></div><span class="hint">${items.length} 条</span></div><div class="review-table"><div class="review-table-head"><span>记录</span><span>来源批次</span><span>问题</span><span>操作</span></div>${items.map((item) => { const candidateNames = (item.candidates || []).map((candidate) => `${candidate.customerName}（${candidate.twCode}）`).join(" / "); const detail = item.detail?.message || item.detail || (item.category === "ambiguous" ? `候选：${candidateNames || "多个客户"}` : item.category === "unmatched" ? "系统暂未找到对应客户" : "需要人工确认"); return `<article class="review-row"><div><strong>${esc(reviewName(item))}</strong><small>${esc(item.pinyinName && item.customerName ? `${item.pinyinName} → ${item.customerName}` : item.customerCode || "" )}</small></div><div><strong>${esc(item.filename)}</strong><small>${fmt(item.importedAt)} · ${esc(reviewProfileLabel(item.profile))}</small></div><div><span class="review-category ${esc(item.category)}">${esc(item.categoryLabel || item.category)}</span><small>${esc(String(detail))}</small></div><div class="review-actions">${item.canApply ? `<button class="secondary-btn" data-review-apply="${esc(item.id)}">关联并入</button>` : ""}<button class="secondary-btn" data-review-ignore="${esc(item.id)}">标记已处理</button></div></article>`; }).join("")}</div></section>` : `<section class="section"><div class="empty">目前没有待人工复核记录。以后导入出现冲突、错误或无法匹配时，会自动汇总到这里。</div></section>`}`;
    content.querySelectorAll("[data-review-apply]").forEach((button) => button.addEventListener("click", () => { const item = items.find((entry) => entry.id === button.dataset.reviewApply); if (item) openImportReviewResolver(item); }));
    content.querySelectorAll("[data-review-ignore]").forEach((button) => button.addEventListener("click", () => resolveImportReview(button.dataset.reviewIgnore, "ignore")));
...
function openImportReviewResolver(item) {
  ...
  openModal(`<div class="modal review-resolver-modal"><div class="modal-header"><div><div class="eyebrow">MANUAL REVIEW</div><h3>关联导入记录</h3><span class="hint">${esc(reviewName(item))} · ${esc(reviewProfileLabel(item.profile))}</span></div><button class="close-btn" data-close>×</button></div><div class="modal-body"><div class="review-resolver-note">确认后只会处理这一条记录，不会覆盖其他客户或负责人。</div>${item.profile === "hongan_activity" ? `<div class="field"><label>确认港安顾问</label><input id="review-advisor" value="${esc(item.targetAdvisor || (item.advisors || [""])[0] || "")}" placeholder="填写港安顾问姓名"></div>` : ""}<form id="review-customer-form"><div class="field"><label>选择系统客户</label><div class="review-search-row"><input id="review-customer-search" value="${esc(initial?.customerName || "")}" placeholder="搜索姓名、TW编号或手机号" required><button class="secondary-btn" type="submit">搜索</button></div></div></form><div id="review-customer-results" class="review-customer-results">${initial ? `<button type="button" class="review-customer-option selected" data-review-customer="${esc(initial.customerId)}"><strong>${esc(initial.customerName)}</strong><small>${esc(initial.twCode || initial.customerCode || "")}</small></button>` : `<span class="hint">输入关键词后搜索客户。</span>`}</div></div></div><div class="modal-footer"><button class="secondary-btn" type="button" data-close>取消</button></div></div>`);
  let selectedId = initial?.customerId || "";
  const results = document.querySelector("#review-customer-results");
  const renderResults = (rows) => { results.innerHTML = rows.length ? rows.map((row) => `<button type="button" class="review-customer-option ${selectedId === row.id ? "selected" : ""}" data-review-customer="${esc(row.id)}"><strong>${esc(row.name || row.wechat_nickname || "未命名")}</strong><small>${esc(row.tw_code || row.customer_code || "")}</small></button>`).join("") : `<span class="hint">没有找到可见客户。</span>`; results.querySelectorAll("[data-review-customer]").forEach((button) => button.addEventListener("click", () => { selectedId = button.dataset.reviewCustomer; results.querySelectorAll(".review-customer-option").forEach((node) => node.classList.toggle("selected", node === button)); })); };
  results.querySelectorAll("[data-review-customer]").forEach((button) => button.addEventListener("click", () => { selectedId = button.dataset.reviewCustomer; button.classList.add("selected"); }));
  ...
  document.querySelector("#review-customer-results")?.addEventListener("click", (event) => { const button = event.target.closest("[data-review-customer]"); if (button) selectedId = button.dataset.reviewCustomer; });
  const footer = document.querySelector(".review-resolver-modal .modal-footer"); const applyButton = document.createElement("button"); applyButton.className = "primary-btn"; applyButton.textContent = "确认并入"; footer?.insertBefore(applyButton, footer.firstChild); applyButton.addEventListener("click", () => { if (!selectedId) { toast("请先选择客户"); return; } resolveImportReview(item.id, "apply", selectedId, document.querySelector("#review-advisor")?.value || ""); });
}
async function resolveImportReview(reviewId, action, customerId = "", honganAdvisor = "") { try { await api(`/api/import-reviews/${encodeURIComponent(reviewId)}/resolve`, {method:"POST", body: JSON.stringify({action, customerId: customerId || null, honganAdvisor: honganAdvisor || null})}); closeModal(); toast(action === "apply" ? "已确认并入这条记录" : "已标记为处理"); await renderImportReviews(document.querySelector("#content")); } catch (err) { toast(err.message); } }
