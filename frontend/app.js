const state = {
  ...
  assignmentOwnerId: "", assignmentSourceAdvisorLabel: "", assignmentHonganAdvisor: "", assignmentSelectedCustomerIds: [], importReviewsIncludeResolved: false, importReviewJobId: "",
  viewHistory: [], muskzoomEntryUrl: "https://muskzoom.com",
};
const app = document.querySelector("#app");
const VIEWS = new Set(["shared-import", "placement-dashboard", "directory", "dashboard", "customers", "placement", "priority", "followups", "imports", "reviews", "assignments", "fields", "bindings", "permissions", "audit"]);
...
function renderShell() {
  ...
  railAssistant.dataset.view = "placement-dashboard";
  ...
  document.querySelectorAll("[data-view]").forEach((button) => button.addEventListener("click", () => { if (button.dataset.view === "reviews") state.importReviewJobId = ""; navigate(button.dataset.view); }));
...
async function navigate(view, options = {}) {
  ...
  renderShell(); const title = {dashboard:"工作台", "shared-import":"客户中心 · 共用资料更新", "placement-dashboard":"定增 · 经营分析", directory:"客户中心 · 全部客户", customers:"定增 · 业务客户", placement:"定增", priority:"优先劣后", followups:"我的跟进", imports:"定增 · 资料录入", reviews:"导入复核", assignments:"客户归属", fields:"表头管理", bindings:"顾问绑定", permissions:"权限设置", audit:"审计日志"}[view]; document.querySelector("#page-title").textContent = title;
  const content = document.querySelector("#content"); content.innerHTML = `<div class="empty">正在同步数据...</div>`;
  try { if (view === "shared-import") renderSharedImport(content); else if (view === "directory") await renderCustomerDirectory(content); else if (view === "dashboard") await renderSharedDashboard(content); else if (view === "placement-dashboard") await renderDashboard(content); else if (view === "customers") await renderCustomers(content); else if (view === "placement") await renderBatches(content); else if (view === "priority") await renderPriority(content); else if (view === "followups") await renderUnifiedFollowups(content); else if (view === "imports") renderImports(content); else if (view === "reviews") await renderImportReviews(content); else if (view === "assignments") await renderCustomerAssignments(content); else if (view === "fields") await renderFields(content); else if (view === "bindings") await renderAdvisorBindings(content); else if (view === "permissions") await renderPermissions(content); else if (view === "audit") await renderAudit(content); } catch (err) { content.innerHTML = `<section class="section"><div class="empty">${esc(err.message)}</div></section>`; }
...
async function renderBatches(content) {
  const data = await api("/api/batches"); const canManage = Boolean(state.user.canManageAssignments); const canImport = Boolean(state.user.canImportCustomers && canManage);
  content.innerHTML = `<div class="section-heading"><div><h3>定增批次</h3><p>一个客户可以参加多个批次。目标批次是当前推进计划，参与记录是已经发生或确认过的历史事实。</p></div>${canManage ? `<button class="primary-btn" id="create-batch">＋ 创建批次</button>` : ""}</div><div class="batch-cards">${data.items.length ? data.items.map((batch) => `<button class="batch-card" data-batch-id="${esc(batch.id)}"><div><span class="eyebrow">定增项目</span>${tag(batch.status, toneFor(batch.status))}</div><h3>${esc(batch.name)}</h3><p>${esc(batch.close_date || "截止日期待定")} · ${batch.customer_count || 0} 位相关客户</p><div class="batch-metrics"><span><small>已建参与关系</small><b>${batch.participation_count || 0}</b></span><span><small>当前目标客户</small><b>${batch.target_customer_count || 0}</b></span><span><small>已参与</small><b>${batch.closed_count || 0}</b></span></div><div class="batch-progress"><i style="width:${batch.intent_amount ? Math.min(100, batch.funded_amount / batch.intent_amount * 100) : 0}%"></i></div><footer>实际参与 ${money(batch.actual_amount)} · 未参与 ${batch.lost_count || 0}</footer></button>`).join("") : `<section class="section"><div class="empty">还没有批次。主管可以创建第一个定增批次。</div></section>`}</div><section id="batch-customers"></section>`;
...
async function loadBatchCustomers(batchId, target) {
  if (!target) return;
  try {
    const data = await api(`/api/batches/${encodeURIComponent(batchId)}/participations`);
    const batch = data.batch;
    const canManage = Boolean(state.user.canManageAssignments);
    ...
    target.querySelectorAll("[data-edit-participation]").forEach((button) => button.addEventListener("click", () => {
      const item = participationRows.find((row) => row.participation_id === button.dataset.editParticipation);
      if (item) openBatchParticipantForm(batch, item, () => loadBatchCustomers(batchId, target));
    }));
    target.querySelectorAll("[data-delete-participation]").forEach((button) => button.addEventListener("click", async () => {
      if (!window.confirm("确认移除这条本批参与记录吗？客户资料、负责人和持仓快照不会受到影响。")) return;
      try { await api(`/api/batch-participations/${encodeURIComponent(button.dataset.deleteParticipation)}`, {method:"DELETE"}); toast("已移除本批参与关系"); await loadBatchCustomers(batchId, target); } catch (err) { toast(err.message); }
    }));
    target.scrollIntoView({behavior:"smooth", block:"start"});
  } catch (err) { target.innerHTML = `<section class="section"><div class="result-box error-text">${esc(err.message)}</div></section>`; }
}

function batchParticipantCustomerLabel(item) { return customerDisplayName(item) || item.wechat_nickname || item.customer_code || "未命名客户"; }
function openBatchParticipantForm(batch, item = null, onComplete = null) {
  const editing = Boolean(item);
  const selectedId = editing ? item.id : "";
  const customerLabel = editing ? batchParticipantCustomerLabel(item) : "";
  openModal(`<div class="modal quick-modal batch-participant-modal"><div class="modal-header"><div><h3>${editing ? "编辑本批参与记录" : "添加本批参与客户"}</h3><span class="hint">只记录客户与“${esc(batch.name)}”的关系，不会改变当前负责人或二级市场持仓。</span></div><button class="close-btn" data-close>×</button></div><form id="batch-participant-form"><div class="modal-body">${editing ? `<div class="selected-customer-summary"><small>客户</small><strong>${esc(customerLabel)}</strong><span>${esc(item.tw_code || item.customer_code || "无编号")}</span></div>` : `<div class="field"><label>搜索客户 *</label><div class="batch-customer-search-row"><input id="batch-customer-search" type="search" placeholder="输入姓名、TW 编号、手机号或微信昵称"><button class="secondary-btn" id="batch-customer-search-button" type="button">搜索</button></div><div id="batch-customer-results" class="batch-customer-results"></div><input type="hidden" id="batch-customer-id" name="customerId" value=""></div>`}<div class="form-grid"><div class="field"><label>本批状态 *</label><select name="status">${(state.meta.batchParticipationStatuses || ["已锁定","资金到账","已参与","未参与"]).map((value) => `<option ${value === (item?.participation_status || "已参与") ? "selected" : ""}>${esc(value)}</option>`).join("")}</select></div><div class="field"><label>意向金额 (USD)</label><input name="intentAmount" type="number" min="0" step="any" value="${item?.participation_intent_amount || 0}"></div><div class="field"><label>到账金额 (USD)</label><input name="fundedAmount" type="number" min="0" step="any" value="${item?.participation_funded_amount || 0}"></div><div class="field"><label>实际参与金额 (USD)</label><input name="actualAmount" type="number" min="0" step="any" value="${item?.participation_actual_amount || 0}"></div></div><div class="field"><label>本批备注</label><textarea name="notes" placeholder="可记录本批的确认说明">${esc(item?.participation_notes || "")}</textarea></div></div><div class="modal-footer"><button class="secondary-btn" type="button" data-close>取消</button><button class="primary-btn">${editing ? "保存修改" : "建立参与关系"}</button></div></form></div>`);
  const root = document.querySelector("#batch-participant-form");
  if (!editing) {
    const search = root.querySelector("#batch-customer-search"); const searchButton = root.querySelector("#batch-customer-search-button"); const results = root.querySelector("#batch-customer-results"); const customerId = root.querySelector("#batch-customer-id"); let rows = [];
    const renderResults = () => { results.innerHTML = rows.length ? rows.map((row) => `<button type="button" class="batch-customer-option" data-select-batch-customer="${esc(row.id)}"><span><strong>${esc(batchParticipantCustomerLabel(row))}</strong><small>${esc(row.tw_code ? `TW ${row.tw_code}` : row.customer_code)} · ${esc(row.owner_name || "待分配")}</small></span><span>选择</span></button>`).join("") : `<div class="batch-customer-empty">请输入关键词搜索可见客户。</div>`; results.querySelectorAll("[data-select-batch-customer]").forEach((button) => button.addEventListener("click", () => { const row = rows.find((candidate) => candidate.id === button.dataset.selectBatchCustomer); if (!row) return; customerId.value = row.id; search.value = batchParticipantCustomerLabel(row); results.innerHTML = `<div class="selected-customer-summary"><small>已选择客户</small><strong>${esc(batchParticipantCustomerLabel(row))}</strong><span>${esc(row.tw_code || row.customer_code || "无编号")}</span></div>`; })); };
    const doSearch = async () => { const query = search.value.trim(); if (!query) { toast("请输入客户姓名或 TW 编号"); return; } searchButton.disabled = true; try { const data = await api(`/api/customers?search=${encodeURIComponent(query)}&pageSize=20`); rows = data.items || []; renderResults(); } catch (err) { toast(err.message); } finally { searchButton.disabled = false; } };
    searchButton.addEventListener("click", doSearch); search.addEventListener("keydown", (event) => { if (event.key === "Enter") { event.preventDefault(); doSearch(); } });
  }
  root.addEventListener("submit", async (event) => { event.preventDefault(); const values = Object.fromEntries(new FormData(root)); if (!editing && !values.customerId) { toast("请先搜索并选择客户"); return; } ["intentAmount", "fundedAmount", "actualAmount"].forEach((key) => { values[key] = Number(values[key] || 0); }); try { const url = editing ? `/api/batch-participations/${encodeURIComponent(item.participation_id)}` : `/api/batches/${encodeURIComponent(batch.id)}/participations`; await api(url, {method: editing ? "PATCH" : "POST", body: JSON.stringify(values)}); closeModal(); toast(editing ? "本批参与记录已更新" : "已建立本批参与关系"); if (onComplete) await onComplete(); } catch (err) { toast(err.message); } });
}

function batchImportOptionHeaders(headers, selected = "") { return `<option value="">不导入</option>${headers.map((header) => `<option value="${esc(header)}" ${header === selected ? "selected" : ""}>${esc(header)}</option>`).join("")}`; }
function openBatchImportForm(batch, onComplete = null) {
  openModal(`<div class="modal quick-modal batch-import-modal"><div class="modal-header"><div><h3>批量导入定增参与记录</h3><span class="hint">先选工作表和列，再看匹配结果。不会新建客户，不会修改负责人，也不会写入二级市场持仓。</span></div><button class="close-btn" data-close>×</button></div><div class="modal-body"><div class="batch-import-file-row"><label class="primary-btn"><input type="file" id="batch-import-file" accept=".xlsx,.csv">选择 Excel / CSV</label><span id="batch-import-file-name" class="hint">默认导入到：${esc(batch.name)}</span></div><div id="batch-import-workspace"><div class="empty">选择文件后，系统会先列出工作表。</div></div></div></div>`);
...
function customerTable(rows, editable = false) {
  ...
  return rows.length ? `<table class="customer-grid"><thead><tr><th class="sticky-col">客户</th><th>TW唯一编号</th><th>拼音</th><th>联系方式</th><th>港安顾问<br><small>外部引荐</small></th><th>开户状态</th><th>定增推进</th><th>当前骄阳负责人</th>${fields.map((field) => `<th>${esc(field.label)}</th>`).join("")}<th>最后更新</th></tr></thead><tbody>${rows.map((row) => {
    const displayName = customerDisplayName(row);
    const namePinyin = String(row.name_pinyin || "").trim();
...
async function renderAdvisorBindings(content) {
  ...
  content.insertAdjacentHTML("beforeend", '<section class="section" style="padding:20px"><h3>让绑定应用到已有客户</h3><p>只处理没有负责人、没有原表指派且未进入定增的客户；已有归属一律保留。</p><button id="apply-default-bindings" class="primary-btn">预览已有客户分配</button><div id="default-binding-impact"></div></section>');
  content.querySelector('#apply-default-bindings').onclick = async () => {
    try {
      const plan=await api('/api/advisor-bindings/apply-existing/preview',{method:'POST'});
      const area=content.querySelector('#default-binding-impact');
      area.innerHTML=`<p>可分配 ${plan.count} 位客户</p><div style="max-height:360px;overflow:auto"><table class="p-table"><thead><tr><th>客户</th><th>保险经纪人</th><th>当前负责人</th><th>绑定负责人</th><th>处理</th></tr></thead><tbody>${plan.items.map(r=>`<tr><td>${esc(r.name)}</td><td>${esc(r.broker)}</td><td>${esc(r.beforeOwner)}</td><td>${esc(r.ownerName)}</td><td>${esc(r.reason)}</td></tr>`).join('')}</tbody></table></div><button id="confirm-default-bindings" class="primary-btn" ${plan.count?'':'disabled'}>确认分配这 ${plan.count} 位客户</button>`;
      area.querySelector('button').onclick=async e=>{setBusyButton(e.target,true);try{const r=await api('/api/advisor-bindings/apply-existing/commit',{method:'POST',body:JSON.stringify({token:plan.token})});area.textContent=`已分配 ${r.assignedCount} 位客户，其他归属保持不变。`;}catch(err){toast(err.message);setBusyButton(e.target,false);}};
    } catch(err){toast(err.message);}
  };
...
async function renderPermissions(content) {
  ...
  content.innerHTML = `<div class="notice"><strong>CRM 独立权限：</strong>账号、岗位和业务团队仍由 MuskZoom 维护；这里仅决定该账号在骄阳客户系统中可见的数据范围和可执行的功能，不会改变 MuskZoom 的岗位、团队或聊天质检权限。<br><strong>人工复核员注意：</strong>需要在“导入复核”里搜索并写入任意客户时，数据范围必须选“全量客户”；只勾选“批量导入”仍可能只能看到本人客户。</div><section class="section"><div class="section-header"><div><h3>客户模块权限</h3><span class="hint">数据范围选“继承 MuskZoom”时：商务经理仅本人、部门主管仅本组、管理员与开发者为全量。</span></div><span class="hint">${data.items.length} 个账号</span></div><div class="table-wrap"><table class="permission-table"><thead><tr><th>成员</th><th>MuskZoom 角色 / 团队</th><th>CRM 数据范围</th><th>批量导入</th><th>客户归属</th><th>顾问绑定</th><th>表头管理</th><th>完整导出</th><th>CRM 权限管理</th></tr></thead><tbody>${data.items.map((item) => { const reviewHint = item.canImportCustomers && item.crmScopeMode !== "all" ? `<small class="permission-warning">导入复核需全量</small>` : ""; return `<tr><td><strong>${esc(item.name)}</strong><div class="hint">${esc(item.username)}</div></td><td>${tag(item.roleLabel, roleTone(item.role))}<div class="hint">${esc(item.team)}</div></td><td><select data-permission="scope" data-user="${esc(item.id)}">${scopeOptions(item.crmScopeMode || "inherit")}</select>${reviewHint}</td><td>${permissionSwitch("import", item, item.canImportCustomers, "允许")}</td><td>${permissionSwitch("assignments", item, item.canManageAssignments, "允许")}</td><td>${permissionSwitch("bindings", item, item.canManageAdvisorBindings, "允许")}</td><td>${permissionSwitch("fields", item, item.canManageCustomerFields, "允许")}</td><td>${permissionSwitch("export", item, item.canExportAll, "允许")}</td><td>${permissionSwitch("permissions", item, item.canManageCrmPermissions, "允许")}</td></tr>`; }).join("")}</tbody></table></div></section>`;
...
async function openLegacyFollowForm(selectedCustomerId = "") {
  ...
  let initialCustomer = state.customers.find((row) => row.id === selectedCustomerId) || null;
  if (selectedCustomerId && !initialCustomer) {
    try {
      const detail = await api(`/api/customers/${encodeURIComponent(selectedCustomerId)}`);
      initialCustomer = detail.customer || null;
      if (initialCustomer) state.customers = [initialCustomer, ...state.customers.filter((row) => row.id !== initialCustomer.id)];
    } catch { /* The picker can still search within the user's visible customers. */ }
  }
  openModal(`<div class="modal"><div class="modal-header"><h3>写今日跟进</h3><button class="close-btn" data-close>×</button></div><form id="quick-follow-form"><div class="modal-body"><div class="field"><label>客户 *</label><div class="follow-customer-picker"><div class="follow-search-row"><input id="follow-customer-search-input" type="search" autocomplete="off" placeholder="搜索姓名、TW 编号、手机号或微信昵称" aria-label="搜索客户"><button class="secondary-btn" type="button" id="follow-customer-search">搜索</button></div><p class="follow-search-hint">先搜索，再点击结果选择客户；最多展示 20 条匹配记录。</p><div id="follow-customer-selected" class="follow-customer-selected" hidden></div><div id="follow-customer-results" class="follow-customer-results" aria-live="polite"></div><input type="hidden" id="follow-customer-id" name="customerId" value="${esc(initialCustomer?.id || selectedCustomerId)}"></div></div><div class="form-grid"><div class="field"><label>沟通方式</label><select name="method">${state.meta.followupMethods.map((v) => `<option>${esc(v)}</option>`).join("")}</select></div><div class="field"><label>跟进后阶段</label><select name="stageAfter">${state.meta.stages.map((v) => `<option>${esc(v)}</option>`).join("")}</select></div></div><div class="field"><label>沟通内容 *</label><textarea name="content" required placeholder="记录客户反馈、资金进度和下一步动作"></textarea></div><div class="form-grid"><div class="field"><label>沟通结果</label><input name="outcome"></div><div class="field"><label>下一步动作</label><input name="nextAction"></div><div class="field"><label>下次跟进时间</label><input name="nextFollowupAt" type="datetime-local"></div></div></div><div class="modal-footer"><button class="secondary-btn" type="button" data-close>取消</button><button class="primary-btn">保存跟进</button></div></form></div>`);
  const searchInput = document.querySelector("#follow-customer-search-input");
  const searchButton = document.querySelector("#follow-customer-search");
  const selectedNode = document.querySelector("#follow-customer-selected");
  const resultsNode = document.querySelector("#follow-customer-results");
  const customerIdInput = document.querySelector("#follow-customer-id");
  let selected = initialCustomer;
  let currentRows = state.customers.slice(0, 20);
  const labelFor = (row) => customerDisplayName(row) || row.wechat_nickname || row.customer_code || "未命名客户";
  const metaFor = (row) => [row.tw_code ? `TW ${row.tw_code}` : row.customer_code, row.owner_name || "待分配"].filter(Boolean).join(" · ");
  const renderSelected = () => {
    ...
    selectedNode.innerHTML = `<div><small>已选择客户</small><strong>${esc(labelFor(selected))}</strong><span>${esc(metaFor(selected))}</span></div><button type="button" class="secondary-btn" data-follow-customer-clear>更换</button>`;
    selectedNode.querySelector("[data-follow-customer-clear]").addEventListener("click", () => { selected = null; renderSelected(); searchInput.focus(); });
  };
  const renderResults = (rows) => {
    currentRows = rows || [];
    resultsNode.innerHTML = currentRows.length ? currentRows.map((row) => `<button type="button" class="follow-customer-option ${selected?.id === row.id ? "selected" : ""}" data-follow-customer="${esc(row.id)}"><span><strong>${esc(labelFor(row))}</strong><small>${esc(metaFor(row) || "无编号信息")}</small></span><span class="follow-customer-use">选择</span></button>`).join("") : `<div class="follow-customer-empty">没有找到可见客户，请换姓名、TW 编号或手机号试试。</div>`;
    resultsNode.querySelectorAll("[data-follow-customer]").forEach((button) => button.addEventListener("click", () => {
      selected = currentRows.find((row) => row.id === button.dataset.followCustomer) || null;
      renderSelected();
  ...
  const searchCustomers = async () => {
    ...
    if (!query) { renderResults(state.customers.slice(0, 20)); return; }
    searchButton.disabled = true;
    try {
      const data = await api(`/api/customers?search=${encodeURIComponent(query)}&page=1&pageSize=20`);
      renderResults(data.items || []);
    } catch (err) { toast(err.message); } finally { searchButton.disabled = false; }
  };
  renderSelected();
  renderResults(currentRows);
  searchButton.addEventListener("click", searchCustomers);
  searchInput.addEventListener("keydown", (event) => { if (event.key === "Enter") { event.preventDefault(); searchCustomers(); } });
  document.querySelector("#quick-follow-form").addEventListener("submit", async (event) => {
    event.preventDefault();
    const values = Object.fromEntries(new FormData(event.currentTarget));
    const customerId = values.customerId;
    if (!customerId) { toast("请先搜索并选择客户"); searchInput.focus(); return; }
    delete values.customerId;
    values.nextFollowupAt = values.nextFollowupAt || null;
    try { await api(`/api/customers/${customerId}/followups`, {method:"POST", body: JSON.stringify(values)}); closeModal(); toast("跟进已保存"); await navigate("followups"); } catch (err) { toast(err.message); }
  });
}
...
function renderCustomerRelations() {
  ...
  const participations = state.detail.batchParticipations || [];
  ...
  const participationRows = participations.length
    ? participations.map((entry) => `<div class="participation-row"><div><strong>${esc(entry.batch_name)}</strong><small>${esc(entry.batch_close_date || "截止日期待定")} · ${esc(entry.source_label || "手工录入")}</small></div><div>${tag(entry.status, toneFor(entry.status))}<small>${money(entry.actual_amount)} 实际参与</small></div>${canManage ? `<button class="secondary-btn" data-edit-participation-detail="${esc(entry.id)}">编辑</button>` : ""}</div>`).join("")
    : `<div class="hint">还没有已确认的定增批次参与记录。客户可以参加多个批次，记录会逐条保留。</div>`;
  const panel = document.createElement("section");
  panel.className = "customer-relations";
  panel.innerHTML = `<div class="relation-section"><div class="relation-heading"><div><h3>协同负责人</h3><span class="hint">主负责人：${esc(item.owner_name)}</span></div>${canManage ? `<button class="secondary-btn" data-manage-collaborators>调整</button>` : ""}</div><div class="collaborator-list">${collaboratorText}</div></div><div class="relation-section"><div class="relation-heading"><div><h3>定增参与记录</h3><span class="hint">每个批次一条记录，和二级市场持仓分开保存</span></div></div><div class="participation-list">${participationRows}</div></div><div class="relation-section"><div class="relation-heading"><div><h3>历史持仓快照</h3><span class="hint">按日期保存数量和市值</span></div><button class="secondary-btn" data-add-snapshot>新增快照</button></div><div class="snapshot-list">${snapshotRows}</div></div>`;
  ...
  panel.querySelectorAll("[data-edit-participation-detail]").forEach((button) => button.addEventListener("click", () => {
    const entry = participations.find((candidate) => candidate.id === button.dataset.editParticipationDetail);
    if (entry) openBatchParticipantForm({id: entry.batch_id, name: entry.batch_name}, {...item, ...entry, participation_id: entry.id, participation_status: entry.status, participation_intent_amount: entry.intent_amount, participation_funded_amount: entry.funded_amount, participation_actual_amount: entry.actual_amount, participation_notes: entry.notes}, () => openDetail(item.id));
  }));
}
...
function openAssignFormNew(item, onComplete = null) {
  ...
  openModal(`<div class="modal"><div class="modal-header"><h3>重新分配骄阳负责人</h3><button class="close-btn" data-close>×</button></div><form id="assign-form"><div class="modal-body"><div class="notice">负责人全产品共用：本操作同步更新定增、优先劣后和客户卡片的负责人、可见范围及后续业绩归属。港安顾问保持原记录。当前港安顾问：<b>${esc(item.hongan_advisor || "未填写")}</b></div><div class="field"><label>分配给</label><select name="ownerId" required>${owners.map((owner) => `<option value="${esc(owner.id)}">${esc(owner.name)} · ${esc(owner.team)}</option>`).join("")}</select></div><div class="field"><label>分配原因 *</label><textarea name="reason" required></textarea></div></div><div class="modal-footer"><button class="secondary-btn" type="button" data-close>取消</button><button class="primary-btn">确认分配</button></div></form></div>`);
...
function reviewDetailMessage(item) {
  const detail = item.detail;
  if (detail && typeof detail === "object") return detail.message || detail.reason || detail.detail || "";
  return detail || item.message || item.reason || "";
}
function reviewIssueText(item) {
  if (item.category === "conflict") {
    ...
    return reviewDetailMessage(item) || "系统已有相同客户，本次导入的字段需要人工确认。";
  }
  ...
  return reviewDetailMessage(item) || "这条记录没有成功写入，需要人工检查原始行。";
}

function reviewDecisionMarkup(item) {
  const category = item.category;
  const title = category === "conflict" ? "已有客户，但导入值和系统值不同" : category === "ambiguous" ? "客户身份不唯一" : category === "unmatched" ? "系统还没有找到对应客户" : "这条记录没有安全写入";
  const steps = category === "conflict"
    ? "请先看下方“系统当前值”和“本次导入值”，决定保留系统值，或选择客户后采用导入值。"
    : category === "ambiguous"
      ? "请根据 TW 编号或其他联系方式选中唯一客户；没有确认前不会写入。"
      : category === "unmatched"
        ? "先搜索已有客户；确认确实没有时，使用“先新建客户”，保存后还要再次确认本条导入。"
        : "请查看来源行和错误说明，修正客户资料或选择暂不写入。";
  return `<section class="review-decision-card"><div><span>这条记录需要你决定什么</span><strong>${title}</strong></div><p>${steps}</p><small>当前操作只影响这一条导入记录，不会自动改负责人，也不会把其他记录一起写入。</small></section>`;
}
...
async function renderImportReviews(content) {
  try {
    const reviewQuery = new URLSearchParams({include_resolved: state.importReviewsIncludeResolved ? "true" : "false"});
    if (state.importReviewJobId) reviewQuery.set("job_id", state.importReviewJobId);
    const data = await api(`/api/import-reviews?${reviewQuery}`);
    ...
    content.querySelector("#clear-review-job")?.addEventListener("click", () => { state.importReviewJobId = ""; renderImportReviews(content); });
...
function renderWizardSheetPicker() {
  ...
  workspace.querySelector("#wizard-reset-file")?.addEventListener("click", renderBulkImport);
  workspace.querySelector("#wizard-start-sheets")?.addEventListener("click", async () => {
    if (!wizard.selectedSheets.length) { toast("请至少选择一个需要导入的工作表"); return; }
    wizard.currentIndex = 0;
    await renderWizardSheetConfiguration();
  });
}
async function renderWizardSheetConfiguration() {
  const wizard = state.importWizard;
  const sheetName = wizardCurrentSheet();
  const workspace = wizardWorkspace();
  ...
  try {
    ...
    const preview = wizardPreview();
    if (["hongan_activity", "holding_pinyin"].includes(preview.importProfile)) renderWizardSpecialSheet(preview);
    else renderWizardGenericSheet(preview);
  } catch (err) {
    workspace.querySelector(".section-body").innerHTML = `<div class="result-box error-text">${esc(err.message)}</div><div class="wizard-footer"><button class="secondary-btn" id="wizard-back-sheets">返回工作表选择</button></div>`;
...
async function renderWizardImpact() {
  ...
  const targets = Object.values(config.sourceMap || {}).filter(Boolean);
  if (!targets.length) { toast("请至少选择一列要导入的数据"); return; }
...
async function commitWizardSheet() {
  const wizard = state.importWizard;
  const preview = wizardPreview();
  ...
  try {
    ...
    if (state.user.canImportCustomers) renderImportHistory();
    if (wizard.currentIndex + 1 < wizard.selectedSheets.length) {
      wizard.currentIndex += 1;
      toast(`“${preview.sheetName}”已完成，继续设置下一张表`);
      await renderWizardSheetConfiguration();
      return;
    }
...
function renderWizardComplete() {
  const wizard = state.importWizard;
  const workspace = wizardWorkspace();
  const summary = wizard.results.map(({sheetName, result}) => `<li><b>${esc(sheetName)}</b><span>新增 ${result.created?.length || 0} · 更新 ${result.updated?.length || 0} · 待复核 ${result.conflicts?.length || 0} · 错误 ${result.errors?.length || 0}</span></li>`).join("");
  workspace.querySelector(".section-body").innerHTML = `<section class="wizard-complete"><span>✓</span><h4>已完成本次工作簿导入</h4><p>每个工作表已分别保存为可追溯批次。需要人工确认的记录已汇总到“导入复核”。</p><ul>${summary}</ul><div class="wizard-footer"><button class="secondary-btn" id="wizard-import-again">导入另一份文件</button><button class="primary-btn" id="wizard-open-history">查看导入批次</button></div></section>`;
  workspace.querySelector("#wizard-import-again")?.addEventListener("click", renderBulkImport);
  workspace.querySelector("#wizard-open-history")?.addEventListener("click", () => document.querySelector("#import-history")?.scrollIntoView({behavior: "smooth", block: "start"}));
}
